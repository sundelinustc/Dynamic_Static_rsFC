...
idx <- fseed %>% sapply(function(x) which(froi %in% x)) # 407, 415, 406, 414
...
system.time({
  ...
  M1 <- seq(dim(M)[2]) %>% sapply(myfun_cat, M) %>% t() %>% as.data.frame()
  colnames(M1) <- Mnames
  print("Completed!!! Data concatenation.")
})

# the merged demographic, clinical, & imaging data
df0 <- cbind(df0_demcli, M1) # (2244x10452)
